...
const SkeletonCard = () => (
  <div className='bg-[var(--bg-card)] border border-[var(--border-muted)] rounded-[2rem] overflow-hidden animate-pulse'>
    <div className='h-28 bg-[var(--bg-hover)]'></div>
    <div className='p-5'>
      <div className='flex items-center gap-3 -mt-10 mb-4'>
        <div className='w-16 h-16 rounded-2xl bg-[var(--bg-hover)] border-2 border-[var(--bg-card)] flex-shrink-0'></div>
        <div className='mt-8 flex-1'>
          <div className='h-3 rounded bg-[var(--bg-hover)] mb-2 w-3/4'></div>
          <div className='h-2.5 rounded bg-[var(--bg-input)] w-1/2'></div>
        </div>
      </div>
      <div className='h-2 rounded bg-[var(--bg-input)] w-full mb-1.5'></div>
      <div className='h-2 rounded bg-[var(--bg-input)] w-2/3 mb-4'></div>
      <div className='flex gap-2'>
        <div className='h-2 rounded-full bg-[var(--bg-input)] w-12'></div>
        <div className='h-2 rounded-full bg-[var(--bg-input)] w-12'></div>
        <div className='h-2 rounded-full bg-[var(--bg-input)] w-12'></div>
      </div>
    </div>
...
const EmptyState = ({ icon: Icon, title, sub, cta, onCta }) => (
  <div className='col-span-full py-24 flex flex-col items-center justify-center bg-[var(--bg-hover)]/20 border border-dashed border-[var(--border-muted)] rounded-[3rem] group'>
    <div className='w-24 h-24 bg-[var(--bg-input)] rounded-full flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-700'>
      <Icon size={40} className='text-[var(--text-secondary)]' />
    </div>
    <h3 className='text-lg font-black uppercase tracking-widest text-[var(--text-primary)] mb-2'>{title}</h3>
    <p className='text-[var(--text-secondary)] font-bold text-[10px] uppercase tracking-widest mb-8'>{sub}</p>
    {cta && (
      <button
...
const Amitie = ({ searchQuery, onSearchFocus, onSearchBlur }) => {
  ...
  const renderUserCard = (user, type = 'friend') => {
    ...
    return (
      <div
        key={`${type}-${user.id}`}
        className='group bg-[var(--bg-card)] border border-[var(--border-muted)] rounded-[2rem] overflow-hidden hover:border-purple-500/30 hover:-translate-y-1 hover:shadow-[0_20px_60px_rgba(139,92,246,0.15)] transition-all duration-500 flex flex-col'
      >
        {/* ── Card banner ── */}
        ...
        <div className='px-5 pb-4 flex-1 flex flex-col'>
          <div className='flex items-end justify-between -mt-8 mb-3'>
            <Link to={`/profile/${user.id}`} className='flex-shrink-0'>
              <div className='w-16 h-16 rounded-2xl overflow-hidden border-2 border-[var(--bg-card)] group-hover:border-purple-500/40 transition-colors shadow-xl'>
          ...
          {/* Location */}
          {user.location && (
            <p className='text-[var(--text-secondary)] text-[11px] flex items-center gap-1 mb-2'>
          ...
          {/* Bio */}
          {user.bio ? (
            <p className='text-[var(--text-secondary)] text-[12px] leading-relaxed mb-3 line-clamp-2 flex-1'>
          ...
          <div className='flex items-center gap-3 mb-4 flex-wrap'>
            ...
            {user.mutual_friends > 0 && (
              <span className='flex items-center gap-1 text-[11px] bg-[var(--bg-hover)] text-[var(--text-secondary)] px-2 py-0.5 rounded-full border border-[var(--border-muted)]'>
                <Users size={10} />
            ...
            <div className='mt-auto'>
              {type === 'friend' && (
                <div className='flex gap-2'>
                  <Link
                    to={`/messages?userId=${user.id}`}
                    className='flex-1 flex items-center justify-center gap-2 py-2.5 text-xs font-bold text-[var(--text-primary)] bg-[var(--bg-hover)] hover:bg-[var(--bg-hover)]/80 border border-[var(--border-muted)] hover:border-[var(--border-color)] rounded-2xl transition-all'
                  >
                    <MessageCircle size={14} /> Message
                  </Link>
                  <button
                    className='flex items-center justify-center gap-1 px-3 py-2.5 text-xs font-bold text-[var(--text-secondary)] hover:text-red-400 bg-[var(--bg-hover)] hover:bg-red-500/10 border border-[var(--border-muted)] hover:border-red-500/20 rounded-2xl transition-all'
              ...
              {type === 'pending' && (
                <div className='flex gap-2'>
                  ...
                  <button
                    className='flex items-center justify-center px-3 py-2.5 text-xs font-bold text-[var(--text-secondary)] hover:text-red-400 bg-[var(--bg-hover)] hover:bg-red-500/10 border border-[var(--border-muted)] hover:border-red-500/20 rounded-2xl transition-all'
  ...
  const renderBlockedCard = (user, index) => (
    <div key={`blocked-${user.id}-${index}`}
      className='group bg-[var(--bg-card)] border border-[var(--border-muted)] rounded-[2rem] overflow-hidden hover:border-red-500/20 hover:-translate-y-1 hover:shadow-[0_20px_60px_rgba(239,68,68,0.1)] transition-all duration-500 flex flex-col'
    >
      ...
      <div className='px-5 pb-5 flex-1 flex flex-col'>
        <div className='flex items-end gap-3 -mt-7 mb-3'>
          <div className='w-14 h-14 rounded-2xl overflow-hidden border-2 border-[var(--bg-card)] flex-shrink-0'>
            <Avatar user={user} size='w-14 h-14' />
          </div>
          <div className='mb-1'>
            <h4 className='text-[var(--text-primary)] font-bold text-sm'>{user.prenom} {user.nom}</h4>
            <span className='text-[10px] font-bold text-red-500/70 uppercase tracking-widest'>Bloqué</span>
          </div>
        </div>
        {user.bio && (
          <p className='text-[var(--text-secondary)] text-[12px] leading-relaxed mb-3 line-clamp-2'>{user.bio}…</p>
        )}
        <div className='mt-auto'>
          <button
            className='w-full py-2.5 text-xs font-black uppercase tracking-wider text-[var(--text-secondary)] hover:text-[var(--text-primary)] bg-[var(--bg-hover)] hover:bg-[var(--bg-hover)]/80 border border-[var(--border-muted)] hover:border-[var(--border-color)] rounded-2xl transition-all'
    ...
    <div className='section-content active'>
      <div className='friends-container'>
        ...
        <div className='section-header'>
          <h2 className='text-[var(--text-primary)]'>Amis</h2>
          <div className='flex items-center gap-2'>
            {!isSearching && (
              <button
                className='px-6 py-3 bg-[var(--text-primary)] text-[var(--bg-primary)] text-[10px] font-black uppercase tracking-widest rounded-[1.5rem] shadow-[0_10px_30px_rgba(255,255,255,0.1)] hover:scale-105 active:scale-95 transition-all flex items-center gap-2'
              ...
              {[
                ...
                <button
                  ...
                  className={`relative px-5 py-2.5 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all duration-300 flex items-center gap-2 border ${
                    activeTab === tab.key
                      ? 'bg-[var(--text-primary)] text-[var(--bg-primary)] border-[var(--text-primary)] shadow-[0_0_20px_rgba(255,255,255,0.15)] scale-105'
                      : 'bg-[var(--bg-hover)] text-[var(--text-secondary)] border-[var(--border-muted)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-hover)]/80'
                  }`}
                  onClick={() => setActiveTab(tab.key)}
                >
                  {tab.label}
                  <span className={`text-[9px] px-1.5 py-0.5 rounded-md font-black ${
                    activeTab === tab.key ? 'bg-black/10 text-[var(--bg-primary)]/60' : 'bg-[var(--bg-hover)] text-[var(--text-secondary)]'
                  }`}>{tab.count}</span>
