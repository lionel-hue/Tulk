...
const Signup = () => {
  ...
  const handleApiError = err => {
    console.error('Full error:', err) // Add this for debugging
    ...
    if (err.code === 'ECONNABORTED' || err.message?.includes('timeout')) {
      return {
        message:
          'La requête a expiré. Veuillez vérifier votre connexion internet et réessayer.',
        isNetworkError: true
      }
    }
    ...
    switch (stage) {
      ...
      case 4:
        if (
          ...
          setError(
            'Veuillez entrer un code de vérification valide à 6 chiffres'
          )
          return false
        }
        return true
  ...
  const isNetworkError = errorMessage => {
    return (
      ...
      errorMessage.includes('requête a expiré')
    )
  }
          ...
          <div className='space-y-4'>
            <div className='bg-[#262626] border border-[#363636] rounded-lg p-4'>
              <p className='text-gray-400 text-sm'>
                ...
                <strong>{formData.email}</strong>. Veuillez entrer le code
                ci-dessous pour compléter votre inscription.
              </p>
              {verificationSent && (
